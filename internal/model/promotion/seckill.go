...
type PromotionSeckillActivity struct {
	...
	ConfigIds        types.ListFromCSV[int64] `gorm:"type:varchar(255);comment:秒杀时段 id" json:"configIds"`
...
type PromotionSeckillConfig struct {
	...
	SliderPicUrls datatypes.JSONSlice[string] `gorm:"type:varchar(2000);comment:秒杀轮播图" json:"sliderPicUrls"`
	Status        int                         `gorm:"default:0;not null;comment:状态" json:"status"`
	model.TenantBaseDO
}
