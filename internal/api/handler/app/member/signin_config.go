...
// @Accept json
// @Produce json
// @Success 200 {object} response.Response{data=[]member.AppMemberSignInConfigResp}
...
func (h *AppMemberSignInConfigHandler) GetSignInConfigList(c *gin.Context) {
	...
	respList := lo.Map(list, func(item *memberModel.MemberSignInConfig, _ int) member.AppMemberSignInConfigResp {
		return member.AppMemberSignInConfigResp{
			Day:   item.Day,
			Point: item.Point,
		}
	})
