...
func (h *ConfigHandler) GetConfigKey(c *gin.Context) {
	key := c.Query("key")
	if key == "" {
		c.JSON(400, core.Error(400, "key is required"))
		return
	}
	config, err := h.configSvc.GetConfigByKey(c, key)
	if err != nil || config == nil {
		c.JSON(500, core.Error(500, "config not found"))
		return
	}
	if !config.Visible {
		c.JSON(500, core.Error(500, "不可见的配置，不允许返回给前端"))
		return
	}
	c.JSON(200, core.Success(config.Value))
}
