...
type DeliveryPriceCalculator struct {
	*trade.BasePriceCalculator
	deliveryFreightSvc *trade.DeliveryExpressTemplateService
...
// Calculate 执行运费计算
func (c *DeliveryPriceCalculator) Calculate(ctx context.Context, req *trade.TradePriceCalculateReqBO, respBO *trade.TradePriceCalculateRespBO) error {
	// 只有快递配送才需要计算运费
