...
func (c *CouponPriceCalculator) Calculate(ctx context.Context, req *trade.TradePriceCalculateReqBO, resp *trade.TradePriceCalculateRespBO) error {
	...
	couponBO, foundBO := lo.Find(resp.Coupons, func(item trade.TradePriceCalculateCouponBO) bool {
...
func (c *CouponPriceCalculator) calculateCoupons(coupons []*promotionModel.PromotionCoupon, resp *trade.TradePriceCalculateRespBO) []trade.TradePriceCalculateCouponBO {
	res := make([]trade.TradePriceCalculateCouponBO, 0, len(coupons))
	now := time.Now()

	for _, coupon := range coupons {
		bo := trade.TradePriceCalculateCouponBO{
			ID:                 coupon.ID,
			Name:               coupon.Name,
...
func (c *CouponPriceCalculator) filterMatchCouponOrderItems(items []trade.TradePriceCalculateItemRespBO, coupon *promotionModel.PromotionCoupon) []trade.TradePriceCalculateItemRespBO {
	res := make([]trade.TradePriceCalculateItemRespBO, 0)
	for i := range items {
		item := items[i]
		if !item.Selected {
			continue
...
func (c *CouponPriceCalculator) buildPromotionItems(items []trade.TradePriceCalculateItemRespBO, prices []int) []trade.TradePriceCalculatePromotionItemBO {
	res := make([]trade.TradePriceCalculatePromotionItemBO, len(items))
	for i, item := range items {
		res[i] = trade.TradePriceCalculatePromotionItemBO{
			SkuID:         item.SkuID,
