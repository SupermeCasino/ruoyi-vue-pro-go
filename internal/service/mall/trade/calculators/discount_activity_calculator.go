...
func (c *DiscountActivityPriceCalculator) Calculate(ctx context.Context, req *tradeSvc.TradePriceCalculateReqBO, resp *tradeSvc.TradePriceCalculateRespBO) error {
	// 只处理普通订单
...
// 实现 tradeSvc.SkuPromotionCalculator 接口
func (c *DiscountActivityPriceCalculator) CalculateSkuPromotion(
	ctx context.Context,
	userId int64,
	skuId int64,
	price int,
) (*tradeSvc.SkuPromotionResult, error) {
	// 1. 获取限时折扣活动
	...
	if discountAmount > 0 && discountAmount >= vipAmount {
		// 限时折扣优惠更大
		result := &tradeSvc.SkuPromotionResult{
		...
		// VIP 优惠更大
		return &tradeSvc.SkuPromotionResult{
