...
func (s *BargainHelpService) CreateBargainHelp(ctx context.Context, userID int64, r *req.AppBargainHelpCreateReq) (*promotion.PromotionBargainHelp, error) {
	...
	err := s.q.Transaction(func(tx *query.Query) error {
		...
		if record.Status != consts.BargainRecordStatusInProgress { // 进行中
		...
		if activity.Status != consts.ActivityStatusRun { // 进行中
		...
		if newPrice <= activity.BargainMinPrice {
			...
			newStatus = consts.BargainRecordStatusSuccess // 成功
		}
		...
		if newStatus == consts.BargainRecordStatusSuccess {
